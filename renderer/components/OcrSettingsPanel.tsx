...
export function OcrSettingsPanel() {
  ...
  if (!mounted) {
    return <p className="text-sm text-slate-500">OCR 설정을 불러오는 중…</p>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        {bounds && (
          <p className="text-xs text-slate-500 sm:col-span-2">
            기본 디스플레이 크기: {bounds.width} × {bounds.height} (픽셀).
            Retina 에서 좌표가 어긋나면 값을 조정하세요.
          </p>
        )}
      </div>
      ...
      >
        카카오톡 대화가 보이는 영역만 잡는 것이 좋습니다. 상대방 메시지만 분석을
        켜면 OCR 영역의 오른쪽 일부를 제외해 내 말풍선을 덜 읽습니다. 첫 실행 시
